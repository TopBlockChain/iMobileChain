...
func (set *unconfirmedBlocks) Shift(height uint64) {
	...
	for set.blocks != nil {
		...
		case header.Hash() == next.hash:
			...
			//在合约链中注册已成功进入主链的相关数据
			// if set.chain.GetHeaderByNumber(next.index).Fbtime!="imc_POW"{
			// 	var registerBlockSealArgs=[][]byte{[]byte(params.Unconfirmed_block[header.Number.Uint64()].PrevTime),[]byte(params.Unconfirmed_block[header.Number.Uint64()].CurrentTime),[]byte(header.Number.String()),[]byte(params.Unconfirmed_block[header.Number.Uint64()].MinerCandidates),[]byte(params.Unconfirmed_block[header.Number.Uint64()].MobileMiners)}
			// 	// for key,value:=range params.Unconfirmed_block{
			// 	// 	fmt.Println(key,value.MinerCandidates,value.PrevTime,value.MinerCandidates)
			// 	// }
			// 	err:=set.fbclient.RegisterBlockSeal(registerBlockSealArgs)
			// 	if err!=nil{
            //          fmt.Println("区块注册失败，区块号：",next.index) 
			// 	}else{
			// 		fmt.Println("区块信息注册成功，区块号：",next.index)
			// 	}
			// }
		default:
