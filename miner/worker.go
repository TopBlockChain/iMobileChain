...
// update mainLoop is a standalone goroutine to regenerate the sealing task based on the received event.
...
// push them to consensus engine.  push
...
func (w *worker) SealBlock(chain consensus.ChainReader,block *types.Block, results chan<- *types.Block, stop <-chan struct{})error{
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				log.Warn("Sealing result is not read by miner", "mode", "local", "sealhash")
			}
			close(abort)
		}
		pend.Wait()
...
func (w *worker) mine(block *types.Block, abort chan struct{}, found chan *types.Block) {
	...
	var header = block.Header()
	header = types.CopyHeader(header)
	header.Nonce = types.EncodeNonce(header.Fbtime.Uint64())
	header.MixDigest = common.BigToHash(header.Difficulty)
	search:
	for {
		select {
		case <-abort:
			...
			//fmt.Println("search aborted", "attempts")
			break search

		case <-time.After(wait):			// We don't have to update hash rate on every nonce, so update after after 2^X nonces
			// Seal and return a block (if still needed)
				select {
				case found <- block.WithSeal(header):
					 //fmt.Println("seal work is complete!",header.Number.String())
				case <-abort:
					//fmt.Println("Seal work is  discarded", "attempts",header.Number.String())
				}
				break search
			}
		}
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
  ...
  //此处加入时间计算函数
	...
	//  	PrevBlock=w.chain.GetBlockByNumber(PrevBlock.NumberU64()-uint64(1))
    //     PrevTime=PrevBlock.Fbtime()
	//  }
	//PrevTime= parent.Fbtime()  //父区块所记录时间                                                //定义当前资产链区块
    CurrentBlocknumber := num.Add(num, common.Big1)
	CurrentTime1,err0:=w.fbclient.CurfbNumber()  // .GetLatestMobileMiningEventTime()  //当前最新时间
	if num.Cmp(big.NewInt(1))==0||PrevTime==0{
		PrevTime=CurrentTime1
	}
	if err0==nil {
		...
		//   if err2!=nil{
		// 	fmt.Println("查询移动矿工失败，错误:",err2)
		// 	return
		//  }
		//  params.Unconfirmed_block[CurrentBlocknumber.Uint64()]=params.RegistryBlock{
		// 	  PrevTime: PrevTime,
		// 	  CurrentTime: CurrentTime,
		// 	  MinerCandidates: Resp1, 
		// 	  MobileMiners: Resp2,
		//   }
  ...
  //log.Info("当前矿工的处理时间", "tstamp", tstamp, "duration", duration, "now", time.Now().Unix())
    
	header := &types.Header{
		ParentHash: parent.Hash(),
		Number:     new(big.Int).Set(CurrentBlocknumber),//num.Add(num, common.Big1),
		GasLimit:   core.CalcGasLimit(parent, w.gasFloor, w.gasCeil),
		//Nonce:  types.EncodeNonce(CurrentBlocknumber.Uint64()),
		Extra:      w.extra,
		Time:       big.NewInt(timestamp),
		Fbtime: big.NewInt(int64(CurrentTime)),
	}
	...
	if w.isRunning() {
		...
		case w.taskCh <- &task{receipts: receipts, state: s, block: block, createdAt: time.Now()}:
			...
			log.Info("Commit new mining work", "number", block.Number(), "sealhash", w.engine.SealHash(block.Header()),
				"uncles", len(uncles), "txs", w.current.tcount,"difficulty",block.Difficulty().Uint64(),"gas", block.GasUsed(), "fees", feesEth, "elapsed", common.PrettyDuration(time.Since(start)))
